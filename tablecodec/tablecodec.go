...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	// Key format: tablePrefix (1 byte) + tableID (8 bytes) + recordPrefixSep (2 bytes) + handle (8 bytes)
	// Total length should be RecordRowKeyLen = 19
	...
	// Step 2: decode tableID
	// Skip the table prefix and decode the tableID
	...
	// Step 3: check the record prefix separator
	if !bytes.HasPrefix(k, recordPrefixSep) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record key - %q", key)
	}
	k = k[recordPrefixSepLength:]

	// Step 4: decode handle
	_, handle, err = codec.DecodeInt(k)
	if err != nil {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record key - %q", key)
	}

	// Step 5: return decoded values
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	// Step 1: check if the key is valid
	// Key format: tablePrefix (1 byte) + tableID (8 bytes) + indexPrefixSep (2 bytes) + indexID (8 bytes) + indexValues...
	// Minimum length should be prefixLen + idLen = 11 + 8 = 19
	...
	if !bytes.HasPrefix(k, indexPrefixSep) {
		return 0, 0, nil, errInvalidIndexKey.GenWithStack("invalid index key - %q", key)
	}
	k = k[len(indexPrefixSep):]

	// Step 3: decode indexID
	k, indexID, err = codec.DecodeInt(k)
	if err != nil {
		return 0, 0, nil, errInvalidIndexKey.GenWithStack("invalid index key - %q", key)
	}

	// Step 4: the remaining bytes are indexValues
	indexValues = k
